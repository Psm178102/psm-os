...
import { pageAvaliacoesNps } from './pages/avaliacoes-nps.js';
...
export const ROUTE_GROUP = {
  ...
  '/morimatsu-viabilidade': 'morimatsu',   // v87.71: ⚖️ etapa de viabilidade de leilões (saída · financiabilidade · dívidas)
  // IA
  ...
  '/rh-treinamentos': 'rh', '/rh-recrutamento': 'rh', '/rh-plano': 'rh', '/rh-clima': 'rh', '/rh-avaliacoes': 'rh', '/rh-reunioes': 'rh', '/rh-arch-leg': 'rh', '/rh-funcoes': 'rh',   // abas RH soltas (v81.55 / funcoes+organograma v81.95)
...
(async function boot() {
  ...
  [['/morimatsu', 'visao', 'Morimatsu & Associados'], ['/morimatsu-investidores', 'investidores', 'Morimatsu · Investidores'],
   ...
   ['/morimatsu-viabilidade', 'viabilidade', 'Morimatsu · Viabilidade de Leilões']]
  ...
  // v87.96: a tela Projeção (forecast/summary: valor × peso da etapa) era uma 3ª previsão — aposentada.
  ...
  router.register('/cs-avaliacoes', { render: async (ctx, root) => { setHeader('Avaliações & Feedbacks'); highlight('/cs-avaliacoes'); await pageAvaliacoesNps(ctx, root); } });
  ...
  router.register('/tabela-sp-capital', { render: async (ctx, root) => { setHeader('Tabela de Empreendimentos SP Capital'); highlight('/tabela-sp-capital'); await pageTabelaImoveis(ctx, root, 'spcapital'); } });
  ...
  return `
    <div class="app-shell">
      <aside class="app-sidebar">
        ...
        <button class="sb-link" data-nav="/tabela-sp-capital"><span class="sb-ico">🏙</span> Tabela Empreendimentos SP Capital</button>
        ...
        <button class="sb-link" data-nav="/cs-avaliacoes"><span class="sb-ico">⭐</span> Avaliações & Feedbacks</button>
...
<!-- v87.51: 🏯 MORIMATSU & ASSOCIADOS — menu PRÓPRIO (pedido do Paulo, 07/set): escritório de
     ...
     pela PSM). Boutique do sócio, não é imobiliária — só sócio vê. -->
        ...
        <button class="sb-link" data-nav="/morimatsu-viabilidade"><span class="sb-ico">⚖️</span> Viabilidade de Leilões</button>
        ...
        <button class="sb-link" data-nav="/morimatsu-minutas"><span class="sb-ico">📜</span> Minutas</button>
