...
let _root = null, _scope = null, _items = [], _canEdit = false, _q = '', _editing = null, _busy = false;
...
function rowHTML(it, cor) {
  return `
    <div class="flex items-center" style="justify-content:space-between;gap:10px;flex-wrap:wrap;border:1px solid var(--bd);border-left:3px solid ${cor};border-radius:10px;padding:10px 12px">
      ...
      <div class="flex gap-2" style="align-items:center">
        <a class="btn btn-primary btn-sm" href="${esc(it.url)}" target="_blank" rel="noopener noreferrer">⬇️ Abrir / baixar</a>
        ${_canEdit ? `
    ...
    <div class="card mt-3" style="background:var(--bg-3);border:1px solid var(--bd)">
      ...
      <div class="flex gap-2" style="flex-wrap:wrap">
        ...
        <div style="flex:1;min-width:160px">
          ...
          <datalist id="mn-cats">${meta.cats.map(c => `<option value="${esc(c)}">`).join('')}</datalist>
        </div>
      </div>
      <div class="mt-2">
