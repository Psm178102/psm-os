/* PSM-OS v2 — 🔁 Reativação MAP (v84.40) · Leire · Secretaria de Vendas
   A fila 1-a-1 virou kanban completo na metodologia da casa: base automática
   do RD (FUNIL MAP aberto + parado + com fone), cadência de 40/dia com fila
   priorizada por VALOR, fluxos por situação, 🧠 IA, prazos coloridos,
   follow-up automático por coluna e fiscalização (reativacao_tocada) sozinha.
...
const MOTIVOS = ['não quer mais', 'comprou em outro lugar', 'número errado', 'não responde'];
const hojeStr = () => {
  const d = new Date();
...
async function reload() {
  if (!_host) return;
  _host.innerHTML = '<div class="card"><div class="flex items-center gap-2 muted"><span class="spinner"></span> Carregando a reativação…</div></div>';
  try {
    _d = await api.request('/api/v3/producao/reativacao_kanban');
  } catch (e) {
    _host.innerHTML = `<div class="card"><div class="alert alert-err">${esc(e.message)}</div>
      <div class="tiny muted mt-1">Se for a 1ª vez: a tabela "reativacao_kanban" precisa da migração no Supabase.</div></div>`;
    return;
  }
  render();
}
...
async function post(body, okMsg) {
  ...
  let r = null;
  try {
    r = await api.request('/api/v3/producao/reativacao_kanban', { method: 'POST', body });
...
const diasParado = c => {
  if (!c.parado_desde) return null;
  const d = Math.floor((Date.now() - new Date(c.parado_desde).getTime()) / 86400000);
  return d >= 0 ? d : null;
};
...
function cardHtml(c) {
  ...
  return `<div class="rk-card" draggable="true" data-id="${esc(c.id)}"
    ...
    <div class="flex" style="gap:4px;flex-wrap:wrap;margin-top:3px">
      ${c.valor ? `<span class="tiny" style="color:#d97706;font-weight:800">💼 ${brl(c.valor)}</span>` : '<span class="tiny muted">sem valor</span>'}
      ${dp !== null ? `<span class="tiny" style="background:#64748b1a;color:#64748b;padding:0 7px;border-radius:999px;font-weight:700">😴 ${dp}d parado</span>` : ''}
...
function render() {
  ...
  let cards = _d.cards || [];
  const naFila = c => ['atrasada', 'hoje'].includes(prazoStatus(c));
  ...
  Object.values(porCol).forEach(l => l.sort((a, b) => peso(a) - peso(b) || (Number(b.valor || 0) - Number(a.valor || 0))));

  _host.innerHTML = `
    <div class="card" style="padding:10px 12px">
      <div class="flex items-center" style="gap:6px;flex-wrap:wrap">
        <h2 class="card-title" style="margin:0;font-size:16px">🔁 Reativação MAP</h2>
        <span class="tiny muted">funil MAP parado · 40/dia por valor · respondeu → gestão fecha</span>
      ...
      ${fluxosAba ? '' : `<div class="flex items-center mt-2" style="gap:6px;flex-wrap:wrap">
        ...
        <input class="input" id="rk-busca" placeholder="🔎 nome ou fone" value="${esc(_busca)}" style="width:160px;padding:4px 9px">
        <span style="margin-left:auto"></span>
      ...
      ${(cfg.colunas || []).map(col => {
        ...
        return `<div class="rk-col" data-col="${esc(col.id)}"
          ...
          <div style="max-height:62vh;overflow-y:auto">
            ...
            ${lista.length > max ? `<button class="btn btn-ghost btn-sm rk-mais" data-col="${esc(col.id)}" style="width:100%">↓ mostrar +40 (${lista.length - max} restantes)</button>` : ''}
          </div>
        </div>`;
      }).join('')}
    </div>`}`;
...
function wireKanban() {
  ...
  $('#rk-hoje').onclick = () => { _fHoje = !_fHoje; render(); };
  $('#rk-gerar').onclick = async () => {
    const r = await post({ action: 'gerar_fila', force: true });
    if (r && r.ok !== false) alert(`🔁 Fila do dia: ${r.total ?? 0} contato(s)\n⏰ ${r.atrasadas || 0} atrasada(s) · 🔁 ${r.followups || 0} follow-up(s) · 📞 ${r.novas || 0} nova(s) (maiores valores primeiro)`);
    else if (r) alert('Cadência desligada na config (⚙️).');
    reload();
  };
  $('#rk-sync').onclick = async () => {
    $('#rk-sync').disabled = true; $('#rk-sync').textContent = '⏳ Sincronizando…';
    const r = await post({ action: 'sincronizar' });
    if (r) alert(`🔄 Sincronizado: ${r.criadas} card(s) novo(s) do funil MAP parado (${r.parado_dias}+ dias)\n📵 ${r.sem_fone} lead(s) ficaram fora por não terem telefone no RD.`);
    reload();
  };
  $('#rk-novo').onclick = async () => {
    const nome = prompt('Nome do contato:');
    if (!nome) return;
    const fone = prompt('Telefone (só números):') || '';
    await post({ action: 'novo', nome, contato: fone });
    reload();
  };
  $('#rk-busca').oninput = e => { _busca = e.target.value; render(); };
  ...
  return new Promise(resolve => {
    const ov = overlay(`
      ...
      <div class="flex mt-2" style="gap:6px;flex-wrap:wrap">
        ${MOTIVOS.map(m => `<button class="btn btn-ghost btn-sm rk-mot" data-m="${esc(m)}">${esc(m)}</button>`).join('')}
      </div>
      <div class="flex mt-2" style="gap:6px">
        <input class="input" id="rk-mot-outro" placeholder="Outro motivo (escreva)" style="flex:1">
        <button class="btn btn-primary btn-sm" id="rk-mot-ok">OK</button>
      </div>
      <div class="flex mt-2" style="justify-content:flex-end"><button class="btn btn-ghost btn-sm" id="rk-mot-x">Cancelar</button></div>`);
    ov.querySelectorAll('.rk-mot').forEach(b => b.onclick = () => { ov.remove(); resolve(b.dataset.m); });
    ov.querySelector('#rk-mot-ok').onclick = () => {
      const t = ov.querySelector('#rk-mot-outro').value.trim();
      if (!t) { alert('Escreva o motivo ou escolha um botão.'); return; }
      ov.remove(); resolve('outro: ' + t);
    };
    ov.querySelector('#rk-mot-x').onclick = () => { ov.remove(); resolve(null); };
  });
}
...
function abrirCard(id) {
  ...
  const ov = overlay(`
    <div class="flex items-center" style="gap:8px">
      ...
      ${c.valor ? `<b style="color:#d97706">${brl(c.valor)}</b>` : ''}
    ...
    <div class="mt-2" style="background:#7c3aed12;border-radius:10px;padding:8px 10px">
      ...
      <div id="rk-ia-out" style="display:none;margin-top:5px">
        <textarea class="input" id="rk-ia-txt" rows="3" style="resize:vertical"></textarea>
        <button class="btn btn-ghost btn-sm mt-1" id="rk-ia-copy">📋 Copiar</button>
      </div>
    </div>
    ...
    <div class="mt-2" style="background:var(--bg-3);border-radius:10px;padding:8px 10px">
      ...
      <div class="flex" style="gap:6px;flex-wrap:wrap;margin-top:3px">
        ...
        <input class="input" id="rk-tini" type="time" value="${esc(c.tarefa?.hora_ini || '')}" style="width:100px">
        <input class="input" id="rk-tfim" type="time" value="${esc(c.tarefa?.hora_fim || '')}" style="width:100px">
        <button class="btn btn-ghost btn-sm" id="rk-tarefa">📅 Agendar</button>
      </div>
    </div>
    <div class="flex mt-3" style="gap:6px;flex-wrap:wrap;justify-content:flex-end">
      ${lvl >= 7 ? '<button class="btn btn-ghost btn-sm" id="rk-del" style="color:#dc2626;margin-right:auto">🗑 Excluir card</button>' : ''}
      <button class="btn btn-primary btn-sm" id="rk-save">💾 Salvar</button>
    </div>`);
  const tags = new Set(c.etiquetas || []);
  ov.querySelectorAll('.rk-tag').forEach(b => b.onclick = () => {
  ...
  ov.querySelector('#rk-ia').onclick = async () => {
    ...
    if (r && r.msg) { ov.querySelector('#rk-ia-out').style.display = ''; ov.querySelector('#rk-ia-txt').value = r.msg; }
  };
  ov.querySelector('#rk-ia-copy').onclick = async () => {
    const t = ov.querySelector('#rk-ia-txt').value;
    try { await navigator.clipboard.writeText(t); } catch (_) { prompt('Copie a mensagem:', t); return; }
    const b = ov.querySelector('#rk-ia-copy'); b.textContent = '✅ Copiado'; setTimeout(() => { b.textContent = '📋 Copiar'; }, 1400);
  };
  ov.querySelector('#rk-save').onclick = async () => {
    const g = s => ov.querySelector(s).value.trim();
    const r = await post({ action: 'editar', id: c.id, nome: g('#rk-nome'), contato: g('#rk-fone'),
                           obs: g('#rk-obs'), etiquetas: [...tags] });
    if (r) { ov.remove(); reload(); }
  };
  ov.querySelector('#rk-tarefa').onclick = async () => {
    const data = ov.querySelector('#rk-tdata').value;
    if (!data) { alert('Escolha a data.'); return; }
    const r = await post({ action: 'tarefa', id: c.id, data,
                           hora_ini: ov.querySelector('#rk-tini').value, hora_fim: ov.querySelector('#rk-tfim').value },
                         '📅 Tarefa criada na Agenda.');
    if (r) { ov.remove(); reload(); }
  };
  const del = ov.querySelector('#rk-del');
...
function fluxoCard(f) {
  ...
  return `<div class="card" style="margin:0 0 10px;padding:12px 14px">
    <div class="flex items-center" style="gap:8px;flex-wrap:wrap">
      ...
      ${_d.can_cfg ? `<button class="btn btn-ghost btn-sm rkf-edit" data-id="${esc(f.id)}" style="margin-left:auto;padding:2px 9px;font-size:11px">✏️ Editar</button>` : ''}
    </div>
...
function fluxoEditor(f) {
  ...
  return `<div class="card" style="margin:0 0 10px;padding:12px 14px;border:1px solid #2563eb55" id="rkf-editor">
    ...
    <div class="flex mt-1" style="gap:6px;flex-wrap:wrap">
      ...
      <input class="input fe-nome" value="${esc(f.nome || '')}" placeholder="Nome do fluxo" style="flex:2;min-width:180px;padding:4px 8px">
    </div>
...
function wireFluxos() {
  _host.querySelectorAll('.rkf-copy').forEach(b => b.onclick = async () => {
  ...
  _host.querySelectorAll('.rkf-edit').forEach(b => b.onclick = () => { _editFluxo = b.dataset.id; render(); });
  const novo = _host.querySelector('#rkf-novo');
  if (novo) novo.onclick = () => { _editFluxo = '__novo__'; render(); };

  const ed = _host.querySelector('#rkf-editor');
  if (!ed) return;
...
function abrirCfg() {
  ...
  const colRow = c => `<div class="flex" style="gap:5px;margin-top:4px;align-items:center" data-cfgcol="${esc(c.id)}">
    ...
    ${!FIXAS.includes(c.id) ? '<button class="btn btn-ghost btn-sm cg-del" type="button" style="color:#dc2626;padding:1px 7px">×</button>' : '<span style="width:30px" class="tiny muted" title="coluna estrutural">🔒</span>'}
  </div>`;
  ...
  ov.querySelector('#cg-save').onclick = async () => {
    ...
    const cadencia = {
      ...
      lote_dia: Number(ov.querySelector('#cd-lote').value) || 40,
      followup_dias: Number(ov.querySelector('#cd-fu').value) || 3,
      parado_dias: Number(ov.querySelector('#cd-parado').value) || 30,
    };
