# PSM-OS Auditoria Linha-por-Linha v27.8

Data: 2026-04-21
Total: 31900+ linhas

## Fix-pass v27.8 (runtime validation via analise estatica profunda)

Bugs encontrados em 6 fluxos criticos:

**Login Firebase + Supabase**:
1. `btoa(senha)` unicode crash L10220/10222 — envolvido `unescape(encodeURIComponent())` + double try/catch
2. `psmAuth.isReady()` ausente em lib — bridge Supabase sempre desativada. Adicionado em psm-supabase.js

**TV mode 4h continuas**:
3. `audioCtx resume` setInterval sem clear — leak em entradas multiplas TV. Adicionado `audioResumeInt` + `audioCtx.close()` no exitTV

**Backup Drive + GitHub**:
4. `uploadBackup` sem retry em 401 (token expirado) — adicionado reauth silencioso + retry 1x
5. Snapshot Drive `version: '2026.04.21-v22m'` stale — lê meta[name=version] dinamico
6. `psm-backup.js` version 27.0 stale — lê meta tag dinamico

**30k linhas + edge cases**:
7. `psmFetch(url, opts, ms=15000)` — wrapper global com AbortController (evita travar em rede ruim)
8. `_cleanDead()` em psm-offline — cleanup items >7 dias (evitam acumulo IndexedDB infinito)

**Versoes atualizadas**:
- sw.js: v27.8 (novo CACHE_VERSION forca reload)
- index.html: meta version v27.8
- psm-backup.js: _version 27.8
- psm-offline.js: _version 27.8
- psm-monitor.js: _version 27.8

## Fix-pass v27.7 aplicado (mantido)
